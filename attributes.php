...
if ($options_status) {
  if ( $options['options_type'] != 1 && $options['options_type'] != 4 && $options['options_type'] != 5 ) {
	...
	$values_query = tep_db_query("select distinct pov.products_options_values_id, pov.products_options_values_name from " . TABLE_PRODUCTS_OPTIONS_VALUES . " pov, " . TABLE_PRODUCTS_OPTIONS_VALUES_TO_PRODUCTS_OPTIONS . " p2p where pov.products_options_values_id = p2p.products_options_values_id and p2p.products_options_id = '" . $option_id . "' and pov.language_id = '" . $languages_id . "' order by pov.products_options_values_name");
