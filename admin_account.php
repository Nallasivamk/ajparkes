  ...
  if (isset($_GET['action']) && $_GET['action']) {
    switch ($_GET['action']) {
      ...
      case 'save_account':
		...
		$admin_email_signature = tep_db_prepare_input($_POST['signature']);
		
		$sign_arr = tep_get_admin_signatures($_SESSION['login_id']); 
		
		
		if(!empty($sign_arr["admin_email_signatures_id"]) && $sign_arr["admin_email_signatures_id"]>0) {
			$sql_data_array = array('signature' => $admin_email_signature,
                                  'modified_date' => 'now()');        
			tep_db_perform(TABLE_ADMIN_EMAIL_SIGNATURES, $sql_data_array, 'update', 'admin_id = \'' . $admin_id . '\'');
		} else {
			$sql_data_array = array('signature' => $admin_email_signature,
								  'admin_id' => $admin_id,
								  'created_date' => 'now()',
                                  'modified_date' => 'now()');  
			tep_db_perform(TABLE_ADMIN_EMAIL_SIGNATURES, $sql_data_array); 
		}
		
		/* SMTP settings update */
		$smtp_settings = tep_get_admin_smtp_settings($_SESSION['login_id']); 
		if(!empty($smtp_settings["admin_id"]) && $smtp_settings["admin_id"]>0) {
			$sql_data_array = array('smtp_host' => tep_db_prepare_input($_POST['smtp_host']),
								  'smtp_port' => tep_db_prepare_input($_POST['smtp_port']),
								  'smtp_auth' => tep_db_prepare_input($_POST['smtp_auth']),
								  'smtp_encryption' => tep_db_prepare_input($_POST['smtp_encryption']),
								  'smtp_username' => tep_db_prepare_input($_POST['smtp_username']),
                                  'smtp_password' => tep_db_prepare_input($_POST['smtp_password']));        
			tep_db_perform(TABLE_ADMIN_SMTP_SETTINGS, $sql_data_array, 'update', 'admin_id = \'' . $admin_id . '\'');
		} else {
			$sql_data_array = array('admin_id' => $admin_id,
								  'smtp_host' => tep_db_prepare_input($_POST['smtp_host']),								  
								  'smtp_port' => tep_db_prepare_input($_POST['smtp_port']),
								  'smtp_auth' => tep_db_prepare_input($_POST['smtp_auth']),
								  'smtp_encryption' => tep_db_prepare_input($_POST['smtp_encryption']),
								  'smtp_username' => tep_db_prepare_input($_POST['smtp_username']),
                                  'smtp_password' => tep_db_prepare_input($_POST['smtp_password']));  
			tep_db_perform(TABLE_ADMIN_SMTP_SETTINGS, $sql_data_array); 
		}
		/* SMTP settings update */
...
//echo "select a.admin_id, a.admin_firstname, a.admin_lastname, a.admin_email_address, a.admin_created, a.admin_modified, a.admin_logdate, a.admin_lognum, g.admin_groups_name from " . TABLE_ADMIN . " a, " . TABLE_ADMIN_GROUPS . " g where a.admin_id= " . $_SESSION['login_id'] . " and g.admin_groups_id= " . $_SESSION['login_groups_id'] . "";
  ...
  $my_account_query = tep_db_query ("select a.admin_id, a.admin_firstname, a.admin_lastname, a.admin_email_address, a.admin_created, a.admin_modified, a.admin_logdate, a.admin_lognum, g.admin_groups_name, ass.smtp_host, ass.smtp_port, ass.smtp_auth, ass.smtp_encryption, ass.smtp_username, ass.smtp_password from " . TABLE_ADMIN . " a INNER JOIN " . TABLE_ADMIN_GROUPS . " g ON (a.admin_groups_id = g.admin_groups_id) LEFT JOIN admin_smtp_settings ass ON (a.admin_id = ass.admin_id) where a.admin_id= " . $_SESSION['login_id'] . " and g.admin_groups_id= " . $_SESSION['login_groups_id'] . "");
...
?>
			  ...
			  <tr class="dataTableRow">
                    <td colspan="2" class="dataTableContent" style="background:#fff;">
						<br/>
						<b>SMTP Settings (GMAIL Users Only)</b>
						<br/>						
						If you set this up, emails sending from admin will synchronized with selected GMAIL's sent items. <br/>
						To achieve this, please follow below steps, <br/>
						<ul>
							<li>
								Step - 1 :  Enable Allow less secure apps in your GMAIL account, Link: https://www.google.com/settings/security/lesssecureapps <br/><br/>
								For more references about this option, please check the following link, https://support.google.com/accounts/answer/6010255
							</li>
							<li>
								Step - 2: Update your SMTP details here.
							</li>
						</ul>						
					</td>
              </tr>
			  ...
			  <tr class="dataTableRow">
                      <td class="dataTableContent"><nobr><?php echo "SMTP Host"; ?>&nbsp;&nbsp;&nbsp;</nobr></td>
                      <td class="dataTableContent"><?php echo tep_draw_input_field('smtp_host', $myAccount['smtp_host']); ?> Ex., smtp.gmail.com</td>
              </tr>
			  
			  <tr class="dataTableRow">
                      <td class="dataTableContent"><nobr><?php echo "SMTP Port"; ?>&nbsp;&nbsp;&nbsp;</nobr></td>
                      <td class="dataTableContent"><?php echo tep_draw_input_field('smtp_port', $myAccount['smtp_port']); ?> Ex., 587 <!--or 465--></td>
              </tr>
			  
			  <tr class="dataTableRow">
                      <td class="dataTableContent"><nobr><?php echo "SMTP Auth"; ?>&nbsp;&nbsp;&nbsp;</nobr></td>
                      <td class="dataTableContent"><?php echo tep_draw_input_field('smtp_auth', $myAccount['smtp_auth']); ?> Ex., true <!--or false--></td>
              </tr>
			  
			  <tr class="dataTableRow">
                      <td class="dataTableContent"><nobr><?php echo "SMTP encryption"; ?>&nbsp;&nbsp;&nbsp;</nobr></td>
                      <td class="dataTableContent"><?php echo tep_draw_input_field('smtp_encryption', $myAccount['smtp_encryption']); ?> Ex., tls <!--or ssl--></td>
              </tr>
			  
			  <tr class="dataTableRow">
                      <td class="dataTableContent"><nobr><?php echo "SMTP Username"; ?>&nbsp;&nbsp;&nbsp;</nobr></td>
                      <td class="dataTableContent"><?php echo tep_draw_input_field('smtp_username', $myAccount['smtp_username']); ?> Ex., derek@ajparkes.com.au</td>
              </tr>
			  
			  <tr class="dataTableRow">
                      <td class="dataTableContent"><nobr><?php echo "SMTP Password"; ?>&nbsp;&nbsp;&nbsp;</nobr></td>
                      <td class="dataTableContent"><?php echo tep_draw_input_field('smtp_password', $myAccount['smtp_password']); ?> Ex., Your gmail password</td>
              </tr>
			  
			  <!-- SMTP Authentication Details -->
					
			<tr class="dataTableRow">
				<td class="main" colspan="2">Signatures: <br/>
				<?php 
					echo tep_load_html_editor();
					echo tep_insert_html_editor("signature",'advanced','200');
					$sign_arr = tep_get_admin_signatures($_SESSION['login_id']); 
					echo cre_cds_draw_textarea_field('signature', 'soft', '100%', 25, stripslashes($sign_arr["signature"])); ?></td>
			</tr>
<?php
    } else {
...
?>                        
			  ...
			  <tr class="dataTableRow">
                      <td class="dataTableContent"><nobr><?php echo "SMTP Host"; ?>&nbsp;&nbsp;&nbsp;</nobr></td>
                      <td class="dataTableContent"><?php echo $myAccount['smtp_host']  ?></td>
              </tr>
			  
			  <tr class="dataTableRow">
                      <td class="dataTableContent"><nobr><?php echo "SMTP Port"; ?>&nbsp;&nbsp;&nbsp;</nobr></td>
                      <td class="dataTableContent"><?php echo $myAccount['smtp_port']; ?></td>
              </tr>
			  
			  <tr class="dataTableRow">
                      <td class="dataTableContent"><nobr><?php echo "SMTP Auth"; ?>&nbsp;&nbsp;&nbsp;</nobr></td>
                      <td class="dataTableContent"><?php echo $myAccount['smtp_auth']; ?></td>
              </tr>
			  
			  <tr class="dataTableRow">
                      <td class="dataTableContent"><nobr><?php echo "SMTP encryption"; ?>&nbsp;&nbsp;&nbsp;</nobr></td>
                      <td class="dataTableContent"><?php echo $myAccount['smtp_encryption']; ?></td>
              </tr>
			  
			  <tr class="dataTableRow">
                      <td class="dataTableContent"><nobr><?php echo "SMTP Username"; ?>&nbsp;&nbsp;&nbsp;</nobr></td>
                      <td class="dataTableContent"><?php echo $myAccount['smtp_username']; ?></td>
              </tr>
			  
			  <tr class="dataTableRow">
                      <td class="dataTableContent"><nobr><?php echo "SMTP Password"; ?>&nbsp;&nbsp;&nbsp;</nobr></td>
                      <td class="dataTableContent">--Hidden--</td>
              </tr>
			  ...
			  <tr class="dataTableRow">
						...
						<?php 
							$sign_arr = tep_get_admin_signatures($_SESSION['login_id']); 
							echo stripslashes($sign_arr["signature"]);
						?></td>
					</tr>		
<?php
  }
