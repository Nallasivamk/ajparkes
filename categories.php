  ...
  switch ($action) {
    ...
    case 'update_category':
      if (isset($_POST[IMAGE_BACK]) || !isset($_POST['sort_order'])) {
		...
		//BOF category code, category type, show on catalog, show on quote
		$categories_code = tep_db_prepare_input($_POST['categories_code']);
		$sql_data_array['categories_code'] = $categories_code;
		$categories_type = tep_db_prepare_input($_POST['categories_type']);
		$sql_data_array['categories_type'] = $categories_type;
		$show_on_catalog = tep_db_prepare_input($_POST['show_on_catalog']);
		$sql_data_array['show_on_catalog'] = $show_on_catalog;
		$show_on_quote = tep_db_prepare_input($_POST['show_on_quote']);
		$sql_data_array['show_on_quote'] = $show_on_quote;
		$workflow_type = tep_db_prepare_input($_POST['workflow_type']);
		$sql_data_array['workflow_type'] = $workflow_type;
		$workflow_default_item = tep_db_prepare_input($_POST['workflow_default_item']);
		$sql_data_array['workflow_default_item'] = $workflow_default_item;
		//EOF category code, category type, show on catalog, show on quote
    ...
    case 'update_product':
	  ...
	  //Design set as default March 28 2017
	  $is_default_custom_product = tep_db_prepare_input($_POST['is_default_custom_product']); 	 
	  if ($is_default_custom_product != $products_old['is_default_custom_product']) $sql_data_array['is_default_custom_product'] = $is_default_custom_product;
	  ...
	  $workflow_sort_order = isset($_POST['workflow_sort_order']) ? tep_db_prepare_input($_POST['workflow_sort_order']) : 0;
      if ($workflow_sort_order != $products_old['workflow_sort_order']) $sql_data_array['workflow_sort_order'] = $workflow_sort_order;
      ...
      if ($products_bundle != $products_old['products_bundle']) $sql_data_array['products_bundle'] = $products_bundle;
	  $sold_in_bundle_only = tep_db_prepare_input($_POST['sold_in_bundle_only']);
      if ($sold_in_bundle_only != $products_old['sold_in_bundle_only']) $sql_data_array['sold_in_bundle_only'] = $sold_in_bundle_only;
	  //EOF Bundle products
    ...
    case 'insert_product':
      if (isset($_POST[IMAGE_BACK]) || !isset($_POST['sort_order'])) {
        ...
        $sql_data_array = array('products_date_available' => $products_date_available,
								...
								'is_default_custom_product' => $is_default_custom_product,
								...
								'workflow_sort_order' => $workflow_sort_order);       
        ...
        if ($_POST['copy_as'] == 'link') {
          ...
          while ($sub_products = tep_db_fetch_array($sub_products_query)) {
            ...
            tep_db_query("insert into " . TABLE_PRODUCTS . " (products_quantity, products_min_order_qty, progressed_quantity, overall_quantity, labour_cost, overhead_cost, material_cost, products_model, products_image, products_image_med, products_image_lrg, products_image_rls2, products_image_sm_1, products_image_xl_1, products_image_sm_2, products_image_xl_2, products_image_sm_3, products_image_xl_3, products_image_sm_4, products_image_xl_4, products_image_sm_5, products_image_xl_5, products_image_sm_6, products_image_xl_6, products_price, products_price1, products_price2, products_price3, products_price4, products_price5, products_price6, products_price7, products_price8, products_price9, products_price10, products_price11, products_price1_qty, products_price2_qty, products_price3_qty, products_price4_qty, products_price5_qty, products_price6_qty, products_price7_qty, products_price8_qty, products_price9_qty, products_price10_qty, products_price11_qty, products_qty_blocks, products_date_added, products_date_available, products_weight, products_status, products_tax_class_id, manufacturers_id, products_parent_id, sort_order, workflow_sort_order, products_qty_days, products_qty_years) values
                        ...
                        '" . tep_db_input($product['products_price1']) . "', '" . tep_db_input($product['products_price2']) . "', '" . tep_db_input($product['products_price3']) . "', '" . tep_db_input($product['products_price4']) . "', '" . tep_db_input($product['products_price5']) . "', '" . tep_db_input($product['products_price6']) . "', '" . tep_db_input($product['products_price7']) . "', '" . tep_db_input($product['products_price8']) . "', '" . tep_db_input($product['products_price9']) . "', '" . tep_db_input($product['products_price10']) . "', '" . tep_db_input($product['products_price11']) . "', '" . tep_db_input($product['products_price1_qty']) . "', '" . tep_db_input($product['products_price2_qty']) . "', '" . tep_db_input($product['products_price3_qty']) . "', '" . tep_db_input($product['products_price4_qty']) . "', '" . tep_db_input($product['products_price5_qty']) . "', '" . tep_db_input($product['products_price6_qty']) . "', '" . tep_db_input($product['products_price7_qty']) . "', '" . tep_db_input($product['products_price8_qty']) . "', '" . tep_db_input($product['products_price9_qty']) . "', '" . tep_db_input($product['products_price10_qty']) . "', '" . tep_db_input($product['products_price11_qty']) . "', '" . tep_db_input($product['products_qty_blocks']) . "',
                         now(), '" . tep_db_input($product['products_date_available']) . "', '" . tep_db_input($product['products_weight']) . "', '0', '" . (int)$product['products_tax_class_id'] . "', '" . (int)$product['manufacturers_id'] . "', '" . $dup_products_id . "', '" . (int)$product['sort_order'] . "', '" . (int)$product['workflow_sort_order'] . "', '" . tep_db_input($product['products_qty_days']) . "', '" . tep_db_input($product['products_qty_years']) . "')");                           
    ...
    <td class="page-container" valign="top">
      <table border="0" width="100%" cellspacing="0" cellpadding="0">
        ...
        if ( isset($action) && ($action == 'new_category' || $action == 'edit_category') ) {
          if ( ($cID) && (!$_POST) ) {
            $categories_query = tep_db_query("select c.categories_id, c.categories_code, cd.categories_name, cd.categories_heading_title, cd.categories_description, cd.categories_head_title_tag, cd.categories_head_desc_tag, cd.categories_head_keywords_tag, c.categories_image, c.parent_id, c.sort_order, c.date_added, c.last_modified,products_group_access, c.products_nav_access, c.categories_type, c.show_on_catalog, c.show_on_quote, c.workflow_type, c.workflow_default_item from " . TABLE_CATEGORIES . " c, " . TABLE_CATEGORIES_DESCRIPTION . " cd where c.categories_id = '" . $cID . "' and c.categories_id = cd.categories_id and cd.language_id = '" . $languages_id . "' order by c.sort_order, cd.categories_name");
			...
			//BOF category type, show_on_catalog, show_on_quote			
			$categories_code = (isset($_POST['categories_code']) ? $_POST['categories_code'] : '' );
			$categories_type = (isset($_POST['categories_type']) ? $_POST['categories_type'] : '' );
			$workflow_type = (isset($_POST['workflow_type']) ? $_POST['workflow_type'] : '' );
			$workflow_default_item = (isset($_POST['workflow_default_item']) ? $_POST['workflow_default_item'] : '' );			
			$show_on_catalog = (isset($_POST['show_on_catalog']) ? $_POST['show_on_catalog'] : '' );
			$show_on_quote = (isset($_POST['show_on_quote']) ? $_POST['show_on_quote'] : '' );
			//EOF category type, show_on_catalog, show_on_quote
          } else {
			...
			//BOF category type, show_on_catalog, show_on_quote, 
			$cInfo->categories_code = (isset($cInfo ->categories_code) ? $cInfo ->categories_code : '' );
			$cInfo->categories_type = (isset($cInfo ->categories_type) ? $cInfo ->categories_type : '' );
			$cInfo->workflow_type = (isset($cInfo ->workflow_type) ? $cInfo ->workflow_type : '' );
			$cInfo->workflow_default_item = (isset($cInfo ->workflow_default_item) ? $cInfo ->workflow_default_item : '' );
			
			$cInfo->show_on_catalog = (isset($cInfo ->show_on_catalog) ? $cInfo ->show_on_catalog : '' );
			$cInfo->show_on_quote = (isset($cInfo ->show_on_quote) ? $cInfo ->show_on_quote : '' );
			//EOF category type, show_on_catalog, show_on_quote
              ...
              <td><fieldset style="display:none;"></fieldset><fieldset><legend><?php echo TEXT_GENERAL_OPTIONS; ?></legend>
                <table width="100%" border="0" cellspacing="3" cellpadding="3" align="center" summary="category image and sort order">
                  <tr valign="top">
                    ...
                    <td width="33%" class="main" align="center">
						<strong><?php echo TEXT_EDIT_SORT_ORDER; ?></strong>&nbsp;<?php echo tep_draw_input_field('sort_order', $cInfo->sort_order, 'size="2"'); ?>
						<!-- BOF category type, show_on_catalog, show_on_quote -->
						<br>
						<strong><?php echo "Categories Code"; ?></strong>&nbsp;						
						<input name="categories_code" id="categories_code" value="<?php echo $cInfo->categories_code; ?>" type="text"> (Like 100000)
						<br>
						
						<strong><?php echo TEXT_INFO_CATEGORY_TYPE; ?></strong>&nbsp;
						<select name="categories_type" id="categories_type">
							<option value="0" <?php if($cInfo->categories_type==0) { echo ' selected '; }; ?>>Buy</option>
							<option value="1"<?php if($cInfo->categories_type==1) { echo ' selected '; }; ?>>Design</option>
							<option value="2" <?php if($cInfo->categories_type==2) { echo ' selected '; }; ?>>Both</option>
						</select>
						<br>
						<strong><?php echo TEXT_INFO_CLIENT_WORKFLOW_TYPE; ?></strong>&nbsp;
						<select name="workflow_type" id="workflow_type">
							<option value=""> --- </option>
							<option value="1" <?php if($cInfo->workflow_type==1) { echo ' selected '; }; ?>>Fixed</option>
							<option value="2"<?php if($cInfo->workflow_type==2) { echo ' selected '; }; ?>>Custom</option>
						</select>
						<br>
						<strong><?php echo "Workflow Default Item"; ?></strong>&nbsp;						
						<input name="workflow_default_item" id="workflow_default_item" value="<?php echo $cInfo->workflow_default_item; ?>" type="text"> (This is based on workflow sort order in products page)
						<br>
						<strong><?php echo TEXT_INFO_SHOW_ON_CATALOG; ?></strong>&nbsp;
						<select name="show_on_catalog" id="show_on_catalog">
							<option value="0"<?php if($cInfo->show_on_catalog==0) { echo ' selected '; }; ?>>No</option>
							<option value="1" <?php if($cInfo->show_on_catalog==1) { echo ' selected '; }; ?>>Yes</option>
						</select>
						<br>
						<strong><?php echo TEXT_INFO_SHOW_ON_QUOTE; ?></strong>&nbsp;
						<select name="show_on_quote" id="show_on_quote">
							<option value="0" <?php if($cInfo->show_on_quote==0) { echo ' selected '; }; ?>>No</option>
							<option value="1"<?php if($cInfo->show_on_quote==1) { echo ' selected '; }; ?>>Yes</option>
						</select>
						<!-- EOF category type, show_on_catalog, show_on_quote -->
						</td>
                  </tr>
          ...
          // new_category_preview (active when ALLOW_CATEGORY_DESCRIPTIONS is 'true')
            ...
            if (isset($_POST)) {
			  ...
			  $workflow_default_item = $_POST['workflow_default_item'];
			  $show_on_catalog = $_POST['show_on_catalog'];
			  $show_on_quote = $_POST['show_on_quote'];
			  //EOF category type, show_on_catalog, show_on_quote
              ...
              $category_query = tep_db_query("select c.categories_id, c.categories_code, cd.language_id, cd.categories_name, cd.categories_heading_title, cd.categories_description, cd.categories_head_title_tag, cd.categories_head_desc_tag, cd.categories_head_keywords_tag, c.categories_image, c.sort_order, c.date_added, c.last_modified, c.categories_type, c.workflow_type, c.workflow_default_item from " . TABLE_CATEGORIES . " c, " . TABLE_CATEGORIES_DESCRIPTION . " cd where c.categories_id = cd.categories_id and c.categories_id = '" . $cID . "'");
          ...
          if ($cre_RCO->get('categories', 'fieldsetpinfo') !== true) {  
              ...
              <td colspan="2"><fieldset style="display:none;"></fieldset><fieldset><legend><?php echo TEXT_GENERAL_OPTIONS; ?></legend>
                <table width="100%" border="0" cellspacing="0" cellpadding="0">
                  <tr valign="top">
                    ...
                    <td width="50%"><table border="0" cellspacing="2" cellpadding="2">
                       <tr>
                        <td class="main"><?php echo TEXT_INFO_DEFAULT; ?></td>
                        <td class="main"><?php
						echo tep_draw_checkbox_field("is_default_custom_product", '1' ,$pInfo->is_default_custom_product);
                        ?></td>
                      </tr>
					  <tr>
					  ...
					  <tr>
                        <td class="main"><?php echo "Workflow Sort order"; ?></td>
                        <td class="main"><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_input_field('workflow_sort_order', $pInfo->workflow_sort_order); ?></td>
                      </tr>
                      <tr>
                        <td colspan="2"><?php echo tep_draw_separator('pixel_trans.gif', '1', '10'); ?></td>
                      </tr>                      
			...
			<?php
				...
				for ($i=0, $n = $bundle_count ? $bundle_count+1 : 3; $i<$n; $i++) {
				  echo '<input type="text" disabled size="30" name="subproduct_' . $i . '_name" value="' . (isset($bundle_array[$i]['name']) ? tep_output_string($bundle_array[$i]['name']) : '') . '">' . "\n";
				  echo '<input type="hidden" size="3" name="subproduct_' . $i . '_id" value="' . (isset($bundle_array[$i]['id']) ? $bundle_array[$i]['id'] : '') . '">' . "\n";
				  echo '<input type="hidden" size="3" name="subproduct_' . $i . '_qty" value="' . (isset($bundle_array[$i]['id']) ? $bundle_array[$i]['qty'] : '') . '">' . "\n";
			...
			<?php
				...
				echo TEXT_ADD_PRODUCT . '<select name="subproduct_selector" onChange="fillCodes()" style="width:200px;">';
				...
				$products = tep_db_query("select pd.products_name, p.products_id, p.products_model from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd where pd.products_id = p.products_id and pd.language_id = '" . (int)$languages_id . "' and p.sold_in_bundle_only='yes' and p.products_id <> " . (int)$_GET['pID'] . $where_str . " order by p.products_model");
				...
				//echo "select pd.products_name, p.products_id, p.products_model from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd where pd.products_id = p.products_id and pd.language_id = '" . (int)$languages_id . "' and p.products_type='0' and p.products_parent_id='0' and p.default_product_id='0' and p.user_id='0' and p.products_id <> " . (int)$_GET['pID'] . $where_str . " order by p.products_model";
              ...
              <tr>
                <td class="main" align="right">
                  ...
                  if ($parentid['products_parent_id'] == 0) {
					...
					echo '<a href="' . tep_href_link("designer_settings.php", 'cPath=' . $cPath . '&pID=' . $pInfo->products_id) . '">' . tep_image_button('button_edit_attributes.gif', "Designer Settings") . '</a>';
                ...
                <table border="0" width="100%" cellspacing="0" cellpadding="2">
                  ...
                  if (is_numeric($bundle_id)) {
                    ...
                    for ($i=0, $n=100; $i<$n; $i++) {
	                  ...
	                  echo '<td class="main"><a href="' . tep_catalog_href_link('product_info.php', 'products_id=' . (int)$bundle_data['products_id']) . '" target="_blank"><strong>&nbsp;(' . $bundle_data['products_model'] . ') '  . $bundle_data['products_name'] . '</strong></a>';
	                  if ($bundle_data['products_bundle'] == "yes") display_bundle($bundle_data['subproduct_id'], $bundle_data['products_price'], $lid);
	                  echo '</td>';
	                  echo '<td align="right" class="main"><strong>&nbsp;' .  $currencies->display_price($bundle_data['products_price'], tep_get_tax_rate($pInfo->products_tax_class_id)) . "</strong></td></tr>\n";
		              ...
		              echo '<tr><td colspan="5" class="main"><strong>' . TEXT_RATE_COSTS . '&nbsp;' . $bundle_sum . '</strong></td></tr><tr><td class="main" colspan="5" style="color:red"><strong>' . TEXT_IT_SAVE . '&nbsp;' . $bundle_saving . "</strong></td></tr>\n";
		            ?>
              </table></td>
            </tr>
          </table>
			  ...
			  <tr>
				<td class="main">
				  ...
				  if ($pInfo->products_bundle == "yes") {
					display_bundle($pid, $pInfo->products_price, $languages[$i]['id']);
				  }
				  ?>
				</td>
			  </tr>
			  <tr>
				<td class="main">
				  ...
				  if ($pInfo->sold_in_bundle_only == "yes") {
					echo '<strong>' . TEXT_SOLD_IN_BUNDLE . '</strong><blockquote>';
                  ...
                  if ($rows > 0) {
                    if (isset($cInfo) && is_object($cInfo)) { // category info box contents
                      ...
                      if ($cre_RCO->get('categories', 'psidebarbuttons') !== true) { 
						...
						$contents[] = array('align' => 'center', 'text' => tep_draw_separator('pixel_trans.gif', '1', '16') . '<a href="' . tep_href_link("designer_settings.php", 'cPath=' . $pInfo->categories_id . '&pID=' . $pInfo->products_id) . '">' . tep_image_button('button_edit_attributes.gif', "Designer Settings") . '</a>');
                      ...
                      $contents[] = array('text' => '<br>' . TEXT_PRODUCTS_PRICE_INFO . ' <b>' . $currencies->format($pInfo->products_price) . '</b><br>' . TEXT_PRODUCTS_QUANTITY_INFO . ' <b>' . $pInfo->products_quantity . '</b><br> Model: <b>' . $pInfo->products_model . '</b>' );
                      // multi-vendor shipping
                      if (defined('MVS_STATUS') && MVS_STATUS == 'true') {
					  ...
					  $related_module_details = "";
					  $qry_products_in_orders = tep_db_query("SELECT orders_id FROM orders_products WHERE products_id = '".$pInfo->products_id."'");
					  if(tep_db_num_rows($qry_products_in_orders)>0) { 
							$related_module_details .= "Orders Entry found: ".tep_db_num_rows($qry_products_in_orders)." Record(s)<br>"; 
							if(tep_db_num_rows($qry_products_in_orders)<=5) {
								while($arr_products_in_orders = tep_db_fetch_array($qry_products_in_orders)){
									$related_module_details .= $arr_products_in_orders["orders_id"].",";
								}
							}
					  }
					  $qry_products_cart = tep_db_query("SELECT customers_id FROM customers_basket WHERE products_id LIKE '%".$pInfo->products_id."%'");
					  if(tep_db_num_rows($qry_products_cart)>0) { $related_module_details .= "<br>Shopping Cart Entry found: ".tep_db_num_rows($qry_products_cart)." Record(s)<br>"; }
					  $qry_products_whishlist = tep_db_query("SELECT customers_id FROM customers_wishlist WHERE products_id = '".$pInfo->products_id."'");
					  if(tep_db_num_rows($qry_products_whishlist)>0) { $related_module_details .= "Wishlist Entry found: ".tep_db_num_rows($qry_products_whishlist)." Record(s)<br>"; }
					  $contents[] = array('text' => '<br>Products relation:<br>'.$related_module_details);
