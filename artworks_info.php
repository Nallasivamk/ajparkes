...
*/
  ...
  if(isset($_GET['cID']) && isset($_GET['aID'])) {
	...
	$customers_email_address = $customers_array['customers_email_address'];
	
	if($artwork_content["orders_id"]>0) {
		$orders_info = tep_get_orders($artwork_content["orders_id"]);		
		$customers_name = $orders_info["customers_name"];
		$customers_email_address = $orders_info['customers_email_address'];
	}
	...
	//artwork content for sending email
		...
		if($notify_customer) {		
			...
			$client_email_content = '<b>Product name</b>: '.$prd_name."<br><br>".
					   '<b>Message</b>: '.stripslashes($comment_to_email)."<br><br>".
					   '<b>Option</b>: '. $option_info['option_name'] ."<br><br>".					   
					   '<b>Artwork Status</b>: ' . $artStatus . "<br><br>" ;		
					   
			$salt = "AJP";
			$encrypted_id = base64_encode($aID . $salt);
			$clink = '<a href="' . tep_href_link('../account_artworks_info.php', "art_id=" . $encrypted_id . "&en=1&time=".(time() + 24*60*60), 'SSL'). '">Click Here</a>';
			
			$client_message = GENERAL_EMAIL_HEADER . sprintf(ARTWORK_CLIENT_FEEDBACK_GREET,$customers_name,$design_name) . $client_email_content . sprintf(ARTWORK_CLIENT_LINK,$clink) . GENERAL_EMAIL_FOOTER;
			...
			$mail->Subject    = sprintf(ARTWORK_CLIENT_FEEDBACK_SUBJECT,$artStatus,$design_name);			
  ...
  <table width="100%"  border="0" align="center" cellpadding="0" cellspacing="0" class="body-table">
    ...
    <td valign="top" class="page-container">
		<table border="0" width="100%" cellspacing="0" cellpadding="0">
          <tr>
            <td class="pageHeading"><?php echo sprintf(HEADING_TITLE,$customers_array['customers_firstname'] . " " . $customers_array['customers_lastname']); ?></td>  
			<td align="right">
				<?php 
					...
					<tr>					  
					  <td width="70%" rowspan="12" valign="top">
							...
							<table id="option-feedback" align="center" width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #DDD;">
								...
								<tr>
									<td>&nbsp;</td>
									<td colspan="2"><font class="blue-txt">Please use only JPG, GIF and PNG files for both Revision and High resolution designs.</font></td>
								</tr>
								<tr>
									<td colspan="3"><?php echo tep_draw_separator('pixel_trans.gif', '1', '5'); ?></td>
								</tr>
								<tr>
								  ...
								  <td><strong>Upload Revision</strong><br><div id="revision_title"></div></td>
									<td>
										<input type="file" name="revision" id="revision" /><br>
										<input type="file" name="rev_hi_res" id="rev_hi_res" />&nbsp;<font class="blue-txt">High Resolution design</font>
									</td>
								</tr>
								<tr>
									<td colspan="3"><?php echo tep_draw_separator('pixel_trans.gif', '1', '5'); ?></td>
					  ...
					  ?>
							  ...
							  <tr>
								...
								<td><?php echo '<a href="mailto:' . $customers_email_address . '"><u>' . $customers_email_address . '</u></a>'; ?></td>
							  </tr>	
							  <tr>
								<td colspan="2"><?php echo tep_draw_separator('pixel_trans.gif', '1', '5'); ?></td>
							  </tr>	
