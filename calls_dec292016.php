...
if (tep_not_null($action)) {
	...
	switch ($action) {
		...
		case 'save':
				...
				tep_redirect(tep_href_link("calls.php", $cLink, 'SSL'));
			...
			<table border="0" width="100%" cellspacing="0" cellpadding="0">
			  <tr>
				<td valign="top">
					...
					<a class="cssButton" href="<?php echo "calls.php?action=new".$cLink; ?>" title="Create Call" style="float:right; clear:both;">Create New Call</a><br/>
...
?>
                ...
                <td class="dataTableContent" align="right">
					...
					<?php if (isset($cInfo) && is_object($cInfo) && ($calls['calls_id'] == $cInfo->calls_id)) { echo tep_image(DIR_WS_IMAGES . 'arrow_right_blue.png', ''); } else { echo '<a href="' . tep_href_link(FILENAME_CALLS, tep_get_all_get_params(array('callsID','cID')) . 'callsID=' . $calls['calls_id'].'&cID='.$calls["customers_id"]) . '">' . tep_image(DIR_WS_IMAGES . 'information.png', IMAGE_ICON_INFO) . '</a>'; } ?>&nbsp;
					  ...
					  if (isset($cInfo) && is_object($cInfo)) {
						$heading[] = array('text' => ''. tep_draw_separator('pixel_trans.gif', '11', '12') .'&nbsp;<br><b>[' . $cInfo->calls_id . ']&nbsp;' . $cInfo->name . '</b>');
							...
							$contents[] = array('align' => 'center',
											...
											'<a href="' . tep_href_link(FILENAME_CALLS, tep_get_all_get_params(array('callsID', 'action', 'cID')) . 'callsID=' . $cInfo->calls_id . '&action=confirm&cID='.$cInfo->customers_id) . '">' . tep_image_button('button_delete.gif', IMAGE_DELETE) . '</a><br>');
