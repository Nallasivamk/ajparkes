  ...
  if (tep_not_null($action)) {
    switch ($action) {
      ...
      case 'update_topic':
		...
		$topic_status = tep_db_prepare_input($_POST['topic_status']);
		$topic_attach_module = tep_db_prepare_input($_POST['topic_attach_module']);
        $sql_data_array = array('sort_order' => $sort_order,
								'topic_status' => $topic_status,
								'topic_attach_module' => $topic_attach_module								
								);
...
// removing topics can be a lengthy process
      ...
      case 'update_article':
        if (isset($_POST['edit_x']) || isset($_POST['edit_y'])) {
		  ...
		  //Modified Sep 06, 2010
          //$articles_date_available = (date('Y-m-d') < $articles_date_available) ? $articles_date_available : 'null';
		  $articles_date_available = (date('Y-m-d') < $articles_date_available) ? $articles_date_available : 'now()';
...
<?php
  ...
  if (isset($_GET['action']) && ($_GET['action'] == 'new_topic_ACD' || $_GET['action'] == 'edit_topic_ACD')) {
    if ( isset($_GET['tID']) && (!$_POST) ) {
      $topics_query = tep_db_query("select t.topics_id, td.topics_name, td.topics_heading_title, td.topics_description, t.parent_id, t.sort_order, t.date_added, t.last_modified,t.topic_status,t.topic_attach_module,t.products_group_access from " . TABLE_TOPICS . " t, " . TABLE_TOPICS_DESCRIPTION . " td where t.topics_id = '" . $_GET['tID'] . "' and t.topics_id = td.topics_id and td.language_id = '" . $languages_id . "' order by t.sort_order, td.topics_name");
		  ...
		  <tr>
			...
			<td class="main">
				<?php 
                    $active = "";
                    $inactive = "checked";
					if (isset($tInfo->topic_status)) {
                      if ($tInfo->topic_status == 1) {
                        $active = "checked";
                        $inactive = "";
                      } else {
                        $active = "";
                    ...
                    echo tep_draw_radio_field('topic_status', 1 ,$active) . 'Active &nbsp;&nbsp;' . tep_draw_radio_field('topic_status', 0 , $inactive) . 'Inactive '; 
                    ?>
			</td>
		  </tr>	
		  
		  <tr bgcolor="#F0F1F1">
                  <td class="main"><?php echo "Attach Module/File"; ?></td>
                  <td class="main">
						<?php
							//swtich between mainpage_module directoies
							if ( file_exists(DIR_FS_TEMPLATES . DEFAULT_TEMPLATE . '/template.php')) {
								
								if(is_dir(DIR_FS_TEMPLATES .DEFAULT_TEMPLATE . '/mainpage_modules/')) {
									$modules_folder = (DIR_FS_TEMPLATES .DEFAULT_TEMPLATE . '/mainpage_modules/');
								} else {
							...
							echo "Mainpage Modules available in " . $modules_folder_2 ."<br>";
							echo tep_draw_pull_down_menu('topic_attach_module', $dirs_array, $tInfo->topic_attach_module, "style='width:150;'");
						?>
				  </td>
				</tr>
