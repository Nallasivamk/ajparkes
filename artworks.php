  ...
  if(isset($_GET['cID'])) {
   ...
   if (isset($_POST['action']) && ($_POST['action'] == 'process') ) {
	...
	if ($error == true) {
		...
		$semail = '<a href="' . str_replace('&amp;', '&', tep_href_link('artworks.php', 'cID=' . $cID, 'SSL')) . '">Click Here</a>';				
		...
		if($notify_customer) {		
			...
			$salt = "AJP";
			$encrypted_id = base64_encode($insert_id . $salt);
			$clink = '<a href="' . tep_href_link('../account_artworks_info.php', "art_id=" . $encrypted_id . "&en=1&time=".(time() + 24*60*60), 'SSL'). '">Click Here</a>';
			
			//exit;
			
			$client_message = GENERAL_EMAIL_HEADER . sprintf($artwork_client_greet,$customers_name) . sprintf($artwork_client_content,$clink,$sale_admin_name) . GENERAL_EMAIL_FOOTER;
			...
			//tep_mail($customers_name, $customers_array['customers_email_address'], ARTWORK_CLIENT_EMAIL_SUBJECT, sprintf($artwork_client_greet,$customers_name) . $artwork_client_content . sprintf($artwork_client_footer,$sale_admin_name), STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS);
