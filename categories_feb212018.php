...
function manage_crm_product($mode,$products_id)
{
	...
	//$url = "https://ajparkes.com.au/ajpcrm/CrmManageProduct.php";

	$url = "https://ajparkes.com.au/ajpcrm/CrmManageProduct.php";

	$fields = "mode=".urlencode($mode)

	."&products_id=".urlencode($products_id);
...
function manage_crm_category($category_id,$category_name)
{
    
	$link = mysql_connect(DB_SERVER, DB_SERVER_USERNAME,DB_SERVER_PASSWORD);
	mysql_select_db(DB_DATABASE, $link);

	$query = "SELECT c.categories_id,c.parent_id,cd.categories_name,cd.categories_description FROM `categories`  AS c 
			INNER JOIN `categories_description` AS cd ON c.categories_id = cd.categories_id
			WHERE cd.language_id = '1'  AND c.categories_id = '$category_id'	";
	$result = mysql_query($query);	
	$family = array();
	$i = 0;
	$fields = "post=true";
	$row = mysql_fetch_assoc($result);
	$fields .= "cre_cat_id=".$category_id
	."&categories_name=".urlencode($row['categories_name'])
	."&categories_desc=".urlencode($row['categories_description'])
	."&parent_id=".urlencode($row['parent_id'])	
	."&cre_id=".$category_id	
  	."&mode=".'update';		
	mysql_close($link);

	$url = "https://ajparkes.com.au/ajpcrm/CrmManageCategory.php";
...
function delete_crm_category($category_id)
{
	$url = "https://ajparkes.com.au/ajpcrm/CrmManageCategory.php";
  ...
  switch ($action) {
    ...
    case 'update_category':
      if (isset($_POST[IMAGE_BACK]) || !isset($_POST['sort_order'])) {
		...
		//BOF category type, show on catalog, show on quote
		...
		//EOF category type, show on catalog, show on quote
...
//***************************************************        
        //update crm category
        //manage_crm_category($categories_id,$_POST['categories_name'][$language_id]);
//*****************************************************
...
//******************************************************      
       //update crm category
      //delete_crm_category($categories_id); 
//*************************************************
...
//****************************************************      
      // manage_crm_product($action,$products_id);  
//*****************************************************      
      if ($isSubProduct) {
    ...
    <td class="page-container" valign="top">
      <table border="0" width="100%" cellspacing="0" cellpadding="0">
        ...
        if ( isset($action) && ($action == 'new_category' || $action == 'edit_category') ) {
          if ( ($cID) && (!$_POST) ) {
            $categories_query = tep_db_query("select c.categories_id, cd.categories_name, cd.categories_heading_title, cd.categories_description, cd.categories_head_title_tag, cd.categories_head_desc_tag, cd.categories_head_keywords_tag, c.categories_image, c.parent_id, c.sort_order, c.date_added, c.last_modified,products_group_access, c.products_nav_access, c.categories_type, c.show_on_catalog, c.show_on_quote from " . TABLE_CATEGORIES . " c, " . TABLE_CATEGORIES_DESCRIPTION . " cd where c.categories_id = '" . $cID . "' and c.categories_id = cd.categories_id and cd.language_id = '" . $languages_id . "' order by c.sort_order, cd.categories_name");
          ...
          // new_category_preview (active when ALLOW_CATEGORY_DESCRIPTIONS is 'true')
            ...
            if (isset($_POST)) {
              ...
              $category_query = tep_db_query("select c.categories_id, cd.language_id, cd.categories_name, cd.categories_heading_title, cd.categories_description, cd.categories_head_title_tag, cd.categories_head_desc_tag, cd.categories_head_keywords_tag, c.categories_image, c.sort_order, c.date_added, c.last_modified, c.categories_type from " . TABLE_CATEGORIES . " c, " . TABLE_CATEGORIES_DESCRIPTION . " cd where c.categories_id = cd.categories_id and c.categories_id = '" . $cID . "'");
